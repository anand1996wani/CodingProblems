...
public class Solution {
    ...
    int TopDownKnapsack(int weightArray[],int valueArray[], int S,int N){
        int array[][] = new int[N + 1][S + 1]; 
        // Recurssive Function Base Condition is initilization in TopDown
        
        
        for(int i = 1; i < N + 1;i++){
            for(int j = 1; j < S + 1;j++){
                if(weightArray[i - 1 ] <= j){
                    array[i][j] = Math.max(valueArray[i - 1] + array[i - 1][j - weightArray[i - 1]], array[i - 1][j]);
                }else{
